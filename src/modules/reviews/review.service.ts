...
interface CreateReviewData {
  userId: string;      // ID của user tạo review
  productId: string;   // ID sản phẩm được đánh giá
  rating: number;      // Điểm đánh giá (1-5)
  commentText?: string | null;  // Nội dung bình luận (tùy chọn)
}
...
interface UpdateReviewData {
  rating?: number;     // Điểm mới (tùy chọn)
  commentText?: string | null;  // Nội dung mới (tùy chọn)
}
...
interface QueryOptions {
  ...
  rating?: number;     // Lọc theo rating cụ thể
  sortBy?: 'createdAt' | 'rating';  // Sắp xếp theo
...
export const reviewService = {
   ...
   */
  async findByProduct(productId: string, options: QueryOptions = {}) {
    const {
      page = 1,
      limit = 10,
      rating,
   ...
   * Cập nhật review
   */
  async update(reviewId: string, userId: string, data: UpdateReviewData) {
    // Find review
