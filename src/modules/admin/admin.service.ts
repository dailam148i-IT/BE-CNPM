...
export const adminService = {
  ...
  async getDashboardSummary(query: DashboardQueryDto): Promise<DashboardSummary> {
    ...
    // -----------------------------------------------------------------------
    const revenueByDate = await prisma.$queryRaw<Array<{ date: Date; revenue: number }>>`
      SELECT 
    ...
    // -----------------------------------------------------------------------
    const categoryStats = await prisma.$queryRaw<Array<{ name: string; value: number }>>`
      SELECT 
        c.name,
  ...
  async listOrders(query: AdminOrdersQueryDto) {
    const { status, paymentStatus, search, fromDate, toDate, page, limit, sortBy, sortOrder } =
      query;
