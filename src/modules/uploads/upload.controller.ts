...
import { uploadService, StorageType } from './upload.service.js';
...
export const uploadController = {
  ...
  async uploadSingle(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const storageType = (req.body.storageType as StorageType) || 'local';
  ...
  async deleteImage(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await uploadService.deleteImage(
        publicId,
        (storageType as StorageType) || 'local'
      );
  ...
  async listMedia(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const storageType = (req.query.storageType as 'all' | 'local' | 'cloudinary') || 'all';
