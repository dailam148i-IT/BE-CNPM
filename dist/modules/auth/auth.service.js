...
export const authService = {
    ...
    async login(data) {
        ...
        const accessToken = jwt.sign({ userId: user.id, role: user.role.name }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '15m' } // 15 phút
        );
        // 5. Tạo Refresh Token
        const refreshToken = jwt.sign({ userId: user.id }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '7d' } // 7 ngày
        );
    ...
    async refreshToken(refreshToken) {
        ...
        const newAccessToken = jwt.sign({ userId: user.id, role: user.role.name }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '15m' } // 15 phút
        );
        const newRefreshToken = jwt.sign({ userId: user.id }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '7d' } // 7 ngày
        );
        // 7. Lưu session mới
