...
export const uploadService = {
     ...
     */
    async uploadImage(filePath, folder = 'teashop', storageType = 'local') {
     ...
     */
    async uploadMultiple(filePaths, folder = 'teashop', storageType = 'local') {
        const results = await Promise.all(filePaths.map((filePath) => this.uploadImage(filePath, folder, storageType)));
        return results;
    },
    /**
     ...
     */
    async saveLocal(filePath, folder) {
        try {
     ...
     */
    async deleteImage(publicId, storageType) {
        if (storageType === 'cloudinary') {
     ...
     */
    async deleteMultiple(publicIds, storageType) {
        const results = await Promise.all(publicIds.map((id) => this.deleteImage(id, storageType)));
        return results;
    },
    // ============================================================================
     ...
     */
    async listAllMedia(folder, storageType = 'all') {
        const results = [];
        // Lấy từ local
     ...
     */
    async listCloudinaryMedia(folder) {
        const results = [];
        try {
