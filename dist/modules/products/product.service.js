...
export const productService = {
    ...
    async findAll(options) {
        const { page = 1, limit = 12, status, categoryId, categorySlug, minPrice, maxPrice, search, sortBy = 'createdAt', sortOrder = 'desc', } = options;
        // ====================================
        // BUILD WHERE CLAUSE
        // ====================================
        const where = {};
        ...
        if (minPrice || maxPrice) {
            where.price = {};
            if (minPrice)
                where.price.gte = minPrice; // >= minPrice
            if (maxPrice)
                where.price.lte = maxPrice; // <= maxPrice
        }
