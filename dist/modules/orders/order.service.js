...
export const orderService = {
    ...
    async create(userId, data) {
        ...
        for (const item of cart.items) {
            ...
            if (item.quantity > product.stockQuantity) {
                throw new BadRequestError(`Sản phẩm "${product.name}" chỉ còn ${product.stockQuantity} trong kho`);
            }
            // Lấy giá
        ...
        await notificationService.notifyNewOrder(order.id, shippingAddress, // Using shipping address as customer identifier
     ...
     */
    async getById(orderId, userId, isAdmin = false) {
     ...
     */
    async findAll(query, userId, isAdmin = false) {
        const { status, paymentStatus, fromDate, toDate, page, limit, sortBy, sortOrder, } = query;
        // Build where clause
        const where = {};
        // User chỉ xem được orders của mình
        if (!isAdmin) {
    ...
    async updateStatus(orderId, data) {
        ...
        if (!validNextStatuses.includes(newStatus)) {
            throw new BadRequestError(`Không thể chuyển trạng thái từ ${currentStatus} sang ${newStatus}. ` +
     ...
     */
    async cancel(orderId, userId, isAdmin = false, data) {
        ...
        if (!cancellableStatuses.includes(order.status)) {
            throw new BadRequestError(`Không thể hủy đơn hàng có trạng thái ${order.status}. ` +
